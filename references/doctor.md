Report and repair drift between this project's Bright Design artifacts and what the installed version reads: PRODUCT.md, DESIGN.md and its `.bright-design/design.json` sidecar, `.bright-design/config.json`, persisted surface briefs, and the design hook.
...
- **Tool version.** The installed skill is older than the published one. `bright-design context` reports that at boot as `UPDATE_AVAILABLE` and `npx bright-design update` fixes it. Not this command's job.
- **Schema drift.** An artifact was written by an older Bright Design: fields nothing reads, fields now expected, files in retired locations. Mechanical, and this command repairs most of it.
...
```
{{scripts_path}}/bright-design doctor --json
```
...
- **`auto`** carries no decision. Run `{{scripts_path}}/bright-design doctor --fix` once to apply these, then report what it moved in one line. Do not ask permission first, and do not ask about them afterward.
