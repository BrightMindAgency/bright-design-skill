...
Manual `npx bright-design detect` scans use the same project filter config by default: `detector.ignoreRules`, `detector.ignoreFiles`, `detector.ignoreValues`, and `detector.designSystem.enabled`. `hook.enabled` only controls automatic hook execution, not manual CLI scans. Use `npx bright-design detect --no-config ...` for a raw detector run that ignores project config/context. Use `npx bright-design ignores ...` for direct CLI CRUD on the same detector ignores.

Supported harnesses: Claude Code (`.claude/settings.local.json` in the project, which is gitignored so the hook stays machine-local; a hook you move into the shared `settings.json` is honored in place too), Codex (`.codex/hooks.json` in the project), Cursor (`.cursor/hooks.json` in the project), Grok Build (`.grok/hooks/bright-design.json` in the project; requires `/hooks-trust` or `--trust`), and GitHub Copilot (`.github/hooks/bright-design.json` in the project, a team-shared committed file that both the Copilot CLI and the cloud agent read). For the Copilot CLI, repo-level hooks fire once `.github/hooks/bright-design.json` is committed to the repository's default branch.
...
| `on` | Set `enabled: true` in `.bright-design/config.json`, record local hook consent as accepted, and install/repair provider hook manifests when the skill is installed. |
| `off` | Set `enabled: false` in `.bright-design/config.json`. |
...
| `ignore-value <id> <value> [--shared] [--reason "..."]` | Append a rule/value suppression to shared `.bright-design/config.json`. |
| `ignore-value <id> <value> --local [--reason "..."]` | Append a private rule/value suppression to `.bright-design/config.local.json`. |
...
2. Invoke the admin script and pass the user's output through verbatim:
   ...
   ```bash
   {{scripts_path}}/bright-design hooks <action> [args...]
   ```

3. If `<action>` is `off`, follow up with a one-line note: "Done. New edits will not trigger the design hook in this project until you run `bright-design hooks on`."
4. If `<action>` is `on`, follow up with: "Done. The design hook will fire after the next Edit/Write on a UI file."
5. If `<action>` is `ignore-value`, `ignore-file`, or `ignore-rule`, just print the script output. The default scope is shared `.bright-design/config.json`; add `--local` only when the user explicitly asks for a private exception.
6. If `<action>` is `status`, just print the script output. Do not add commentary unless the user asked a follow-up question.

## Triage findings

The hook itself never writes ignore config; every exception goes through `bright-design hooks`. Triage each finding into one of three outcomes:
...
- If the finding line shows an `ignore-value <rule> <value>` pair, pass it to `bright-design hooks ignore-value` with your `--reason`. This writes shared `.bright-design/config.json` by default.
...
- If the finding has no value-specific command, such as `side-tab`, scope that one rule to the file: `ignore-value <id> "*" --file <path>`. Run `npx bright-design detect <path>` first to see what actually fires there.
...
- Prefer config ignores (the commands above) by default; they keep suppressions in one reviewable place. Reach for an inline comment only when the waiver must travel with a single file that leaves the repo (a generated/exported standalone document, an emailed HTML file). The supported marker is `bright-design-disable <rule>` (whole file) or `bright-design-disable-line` / `bright-design-disable-next-line` (one line), in any comment syntax, with an optional reason after `:` or `--`. The detector honors it by default; `--no-inline-ignores` or `--no-config` bypasses it.

Example value-specific exception:

```bash
{{scripts_path}}/bright-design hooks ignore-value overused-font Inter --shared --reason "User confirmed Inter is intentional"
```

Example self-served exception, with the evidence named:

```bash
{{scripts_path}}/bright-design hooks ignore-value bounce-easing bounce-ball --shared --reason "Agent: literal ball-bounce animation, bounce easing is the subject"
```

Example whole-rule font exception:

```bash
{{scripts_path}}/bright-design hooks ignore-rule overused-font --all-values --reason "User asked to ignore overused fonts generally"
```
...
```bash
{{scripts_path}}/bright-design hooks ignore-value design-system-font-size "*" --file "src/overlay/widget.js" --reason "Injected widget builds its own type scale; DESIGN.md's ramp describes the site"
```

Example whole-file exception, for a file that is out of scope entirely:

```bash
{{scripts_path}}/bright-design hooks ignore-file "src/legacy/Card.tsx"
```

## Constraints

- Never modify `.bright-design/config.json` or `.bright-design/config.local.json` by hand from this command. Always go through `bright-design hooks` so writes stay validated and the file shape stays consistent. One exception: `detector.extensions` has no admin action, so when the user asks to cover a template stack, edit that one field in `.bright-design/config.json` directly and leave the rest of the file untouched.
- Do not edit the launcher or the binary behind `bright-design hook` and `bright-design hook-before-edit` from this flow. Those are skill plumbing.
...
- The hook is bundled with the Bright Design skill and installed through project-local manifests: `.claude/settings.local.json`, `.codex/hooks.json`, `.cursor/hooks.json`, and `.github/hooks/bright-design.json`. On Codex, the user must approve the hook via `/hooks` the first time. On Cursor, confirm hooks are enabled under Settings -> Hooks. On GitHub Copilot, the CLI loads `.github/hooks/bright-design.json` once it is committed to the repository's default branch, and the cloud agent reads it from the repo directly.

## Failure modes

- If `.bright-design/config.json` or `.bright-design/config.local.json` is unreadable or malformed, the hook ignores that file and uses the remaining valid config/defaults. `bright-design hooks status` will show malformed files as ignored.
- If the user asks to "disable the hook" globally, lead with `bright-design hooks off` (persistent for this project; writes `hook.enabled: false` to config). The legacy `IMPECCABLE_HOOK_DISABLED=1` env var also works as a one-shot override that follows the shell.
